...
var fs = require("fs");
var companies = JSON.parse(
  fs.readFileSync("./src/_data/companies.json").toString()
);
  ...
  if (!files || !files.length) {
    result += `<tr dir="rtl">
              <td colspan="3" class="lost-text" style="text-align-last:center">
                <p style="margin:3em 0;">${changeLastSentence(
                  "در حال حاضر هیچ نسخه‌ای از این انتشار موجود نداریم! این عنوان در معرض فراموشی همیشگی است. آن را دارید؟"
                )}</p>
              </td>
            </tr>`;
  }
  if (files) {
    files.forEach(function (file) {
      result += `<tr>
              ...
              <td title="${file.file_size} Bytes">${formatBytes(
        file.file_size
      )}</td>`;
      if (game.selling) {
        ...
        if (file.tgchannel) {
          result += encodeURI(`https://t.me/${file.tgchannel}`);
        } else {
          result += `https://archive.org/download/${encodeURIComponent(
            game.ia_id
...
async function otherreleases(game, games) {
  ...
  games.forEach(function (ogame) {
    if (game === ogame.data.game) {
      return;
    }
    if (hasCommonTitles(game, ogame.data.game)) {
      result += `<li><a href="${ogame.url}">${genGameObjectTitle(
        ogame.data.game
      )}</a></li>`;
    }
  });
  if (result != "") {
    result =
      '<h2>انتشارهای دیگر این بازی</h2><ul class="other-releases">' +
      result +
      "</ul>";
  }
  return result;
...
module.exports = function (eleventyConfig) {
  ...
  eleventyConfig.addNunjucksAsyncShortcode(
    ...
    async function (game) {
      ...
      let a = await imageShortcode(
        game.id,
        "./src/games/" + game.id + "/photos/" + game.photos[0].path,
        "",
        200,
        true
      );
  ...
  eleventyConfig.addPairedShortcode(
    ...
    function ref(content, id, description) {
      ...
      if (id === "") {
        l = 0;
        if (REFRENCE_MAP[this.page.url]) {
          l = Object.keys(REFRENCE_MAP[this.page.url]).length;
        }
        id = (l + 1).toString();
      }
  ...
  eleventyConfig.addPassthroughCopy("src/opg-vip-contribouters.jpg");
  ...
  eleventyConfig.addFilter("toPersianDigits", function (value) {
    return value.toLocaleString("fa-IR");
  });
  eleventyConfig.addFilter("contribute_type", function (value) {
    switch (value) {
      case "game":
        return "تهیه‌ی بازی";
      case "cover":
  ...
  eleventyConfig.addFilter("consolelog", function (value) {
    console.log("startttttttttttttttt");
    console.log(value);
    console.log("tamammmmmmmmmmmmmmmmm");
    return "";
    exit();
  });
  eleventyConfig.addShortcode("today_date", function () {
  ...
  eleventyConfig.addShortcode("contribute_link", function () {
    return contribute_link;
  });
