...
<template>
  ...
  <div class="mt-8 flex flex-col md:flex-row-reverse gap-6">
    ...
    >
      ...
      <ul class="text-xs mt-4">
        <li class="flex items-center gap-2">
          ...
          <span class="text-gray-500 dark:text-gray-400">
            {{
              ...
              game.games[0]["title_" + locale].join($t("joinCharacter"))
            }}
          </span>
        </li>
        <li class="flex items-center mt-2 gap-2">
          ...
          <span
            ...
            >{{
              ...
              game.games[0]["title_" + (locale == "en" ? "fa" : "en")].join(
                locale == "en" ? "، " : ", "
              )
            }}</span
          >
        ...
        <li
          v-if="game?.release_dates?.length"
          class="flex items-center mt-2 gap-2"
        >
          <strong class="py-1 whitespace-nowrap">{{
            $t("releaseDate")
        ...
        <li
          v-if="game['released_on_' + locale]?.length"
          class="flex items-center mt-2 gap-2"
        >
          <strong class="py-1 whitespace-nowrap">{{ $t("relasedOn") }}</strong>
          <div class="flex-1 h-px bg-gray-300 dark:bg-neutral-700"></div>
          <span class="text-gray-500 dark:text-gray-400">{{
            game["released_on_" + locale].join($t("joinCharacter"))
          }}</span>
        </li>
        ...
        <li
          class="bg-gray-100 dark:bg-gray-900 rounded mt-4 py-2 px-2"
          dir="rtl"
          style="text-align: right"
        >
          <strong
            class="text-medium text-gray-800 dark:text-gray-100 rounded-xl"
            >شامل:</strong
          >
          ...
          >
            <tr v-if="game.slug === 'mountaineer-hi-vu'">
              <th class="bg-indigo-500 text-white p-4" colspan="4">
                فایل این اثر به درخواست سازنده از دسترس خارج شده است.
              </th>
            </tr>
            ...
            <tr>
              ...
              <th scope="col" class="py-3" v-if="game.ia_id"></th>
            </tr>
          </thead>
          <tbody>
            <tr
              v-for="(file, index) in game.files || game.tgfiles"
              :key="index"
  ...
  <div class="flex gap-2 flex-wrap justify-center md:justify-start">
    <a
      v-for="photo in game.photos"
      target="_blank"
      :href="img('/gamePhotos/' + game.id + '/' + photo.id + '.jpg')"
      ><nuxt-img
        class="h-full w-60"
        width="264"
        :src="'/gamePhotos/' + game.id + '/' + photo.id + '.jpg'"
    /></a>
  </div>
